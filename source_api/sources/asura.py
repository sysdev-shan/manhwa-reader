...
Asura Scans uses a custom Next.js / React frontend backed by a REST API at
https://gg.asuracomic.net/api/.  This source uses that API directly because
it is cleaner and more stable than HTML scraping of a React app.
...
GET  https://asuracomic.net/series/{slug}/{chapter_id}/  (HTML, images embedded)
"""
...
class AsuraScansSource(HttpSource):
    ...
    No external server required — communicates directly with Asura's own API.
    """
    ...
    def _manga_from_entry(self, entry: dict) -> SManga:
        ...
        # Thumbnail may be a relative path
        ...
        return SManga(
            ...
            status=status,
            thumbnail_url=thumb,
            initialized=bool(entry.get("description") or entry.get("synopsis")),
        )
    ...
    async def get_chapter_list(self, manga: SManga) -> list[SChapter]:
        try:
            ...
            # Some slugs use the series detail endpoint which includes chapters
        ...
        for entry in entries:
            ...
            ch_num_raw = entry.get("chapter") or entry.get("chapterNumber") or -1
            try:
            ...
            chapters.append(
                SChapter(
                    # Store "slug/chapter_id" so get_page_list can build the URL
    ...
    async def get_page_list(self, chapter: SChapter) -> list[Page]:
        """
        Fetch the chapter page and collect all image URLs.

        Asura Scans embeds images as <img> tags inside the chapter reader div.
        URL format: https://asuracomic.net/series/{manga_slug}/{chapter_id}/
        """
        ...
        # Images live in the main reading container
        ...
        if container:
            ...
            # Broad fallback: any large image likely from CDN
            imgs = [
                img for img in soup.find_all("img")
                if img.get("src", "").startswith("http") and "thumb" not in img.get("src", "")
            ]
